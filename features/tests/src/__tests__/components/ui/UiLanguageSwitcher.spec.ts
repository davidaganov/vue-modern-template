...
describe("UiLanguageSwitcher", () => {
  ...
  it("marks current locale as active", () => {
    ...
    const activeButton = buttons.find((b) => {
      ...
      return cls.includes("bg-primary/20") || cls.includes("lang-switcher__btn--active")
    })
  ...
  it("applies inactive styling to non-active locales", () => {
    ...
    const cls = inactiveButton?.classes().join(" ") || ""
    expect(cls.includes("opacity-60") || cls.includes("lang-switcher__btn")).toBe(true)
  })
})
